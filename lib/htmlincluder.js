...
module.exports = function htmlIncluder() {
  ...
  function getDataFromJsonPath(jsonPath, jsonObj) {
    ...
    if (Array.isArray(result)) ;else if ((typeof result === 'undefined' ? 'undefined' : _typeof(result)) === 'object') result = toSafeJsonString(result);
  ...
  function addRawJsonWhereJsonPath(content, rawJson, jsonParentPath) {
    ...
    if (content.length > 1) {
      ...
      content = content.map(function (fragment, ndx, arr) {
        if (fragment.indexOf('<!--#') === 0 && hasTagAttribute(jsonPathAttribute, fragment) && !hasTagAttribute("rawJson", fragment)) {
  ...
  function processIf(file, ndx, arr) {
    ...
    if (hasTagAttribute(jsonPathAttribute, content)) jsonPath = getTagAttribute(jsonPathAttribute, content);
  ...
  function processEach(file, ndx, arr) {
    ...
    if (jsonPath) jsonData = getDataFromJsonPath(jsonPath, rawJson);
    if (hasTagAttribute('count', content)) count = parseInt(getTagAttribute('count', content), 10);
      ...
      if (jsonData && Array.isArray(jsonData)) {
        for (var j = 0; j < tmp.length; j++) {
          if (tmp[j].indexOf('<!--#data') === 0 && !hasTagAttribute('rawJson', tmp[j])) {
            if (Array.isArray(jsonData)) {
              tmp[j] = addTagAttribute('rawJson', tmp[j], toSafeJsonString(jsonData[_i]));
            }
          ...
          if ((tmp[j].indexOf('<!--#wrap') === 0 || tmp[j].indexOf('<!--#insert') === 0) && !hasTagAttribute('rawJson', tmp[j]) && _typeof(jsonData[_i]) === 'object') {
            tmp[j] = addTagAttribute('rawJson', tmp[j], toSafeJsonString(jsonData[_i]));
          }
        }
      }
  ...
  function processWraps(file, ndx, arr) {
    ...
    if (hasTagAttribute(jsonPathAttribute, content)) jsonParentPath = getTagAttribute(jsonPathAttribute, content);

    // see if we have raw json to load from
    if (hasTagAttribute('rawJson', content)) rawJson = getTagAttribute('rawJson', content);

    // get filepath (either relative or absolute)
    filename = pattern === pathPattern ? getTagAttribute(filePathAttribute, arr[ndx]) : getTagAttribute("absPath", arr[ndx]);

    // build filepath if its relative
    fpath = pattern === pathPattern ? buildPathFromRelativePath(file.path, filename) : filename;
  ...
  function processInserts(file) {
    ...
    } else if (hasTagAttribute('absPath', content)) filename = getTagAttribute("absPath", content);

    if (insertFiles[filename]) content = insertFiles[filename].content;else {
      console.error("ERROR: in file " + file.path + ": insert file `" + filename + "` does not exist");
      return "";
    }

    if (rawJson) content = addRawJsonWhereJsonPath(content, rawJson);else if (jsonParentPath) content = appendJsonParentPath(content, jsonParentPath);

    if (file.tmpPath) content = updateRelativePaths(file.tmpPath, content);
  ...
  function processJsonData(tag) {
    ...
    if (hasTagAttribute("default", tag)) defaultValue = getTagAttribute('default', tag);
    ...
    while (splitArr.length > 1) {
      ...
      for (var i = 0; i < splitArr.length; i++) {
        ...
        if (fragment.indexOf('<!--#') === 0) {
          ...
          if (hasPath) tempDirectory = buildPathFromRelativePath(pathStack, getTagAttribute(filePathAttribute, fragment));else if (hasAbsPath) {
      ...
      // debug features
      if (devOptions.printIterations) console.log(content);

      if (devOptions.limitIterations) {
        itterCount++;
        if (itterCount >= devOptions.limitIterations) break;
      }
    }

    if (devOptions.printResult) console.log(content);
  ...
  includer.hashFile = function hashFile(file) {
    ...
    if (f.name[0] === '_') wrapFiles[f.path] = f;else if (f.name[0] === '-') insertFiles[f.path] = f;else pageFiles.push(f);
  };
