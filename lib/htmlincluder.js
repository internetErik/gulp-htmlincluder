...
module.exports = (function htmlIncluder() {
	...
	function processWrapsJson(file, ndx, arr) {
		...
		if(rawJson) {
			content[0] = replaceJsonPathWithRawJson(content[0], rawJson);
			content[1] = replaceJsonPathWithRawJson(content[1], rawJson);
		}
	...
	function replaceJsonPathWithRawJson(content, rawJson) {
		var fndx = -1,
				lndx = -1,
				left = ""
				right = "";

		// remove json path, since rawJson overrides jsonPath
		content = removeTagAttribute("jsonPath", content);
		...
		if(content.length > 1) {
			...
			content = content.map(function(fragment, ndx, arr) {
					if(fragment.indexOf('<!--#') === 0 && hasTagAttribute(jsonPathAttribute, fragment)) {
