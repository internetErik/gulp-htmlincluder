...
module.exports = (function htmlIncluder() {
	...
	function addRawJsonWhereJsonPath(content, rawJson) {
		var fndx = -1,
				lndx = -1,
				left = ""
				right = "";
	...
	function processEach(file, ndx, arr) {
		...
		if(hasTagAttribute('count', content))
			...
			console.error("ERROR: in file " + file.path + ": <!--#each . . . --> with no count");
		}

		endNdx = findIndexOfClosingTag('<!--#each', '<!--#endeach', ndx, arr);

		for(var i = ndx + 1; i <= endNdx; i++) {
			// if there is an inner each, we need to do these first
			if(arr[i].indexOf('<!--#each') === 0) {
				console.log("found inner each")
				return content;
			}
		}
		...
		if(endNdx === (ndx - 1)) {
			console.warning("WARNING: in file " + file.path + ": <!--#each . . . --> with no content");
			return "";
		}
	...
	function processWrapsJson(file, ndx, arr) {
		...
		if(rawJson) {
			content[0] = addRawJsonWhereJsonPath(content[0], rawJson);
			content[1] = addRawJsonWhereJsonPath(content[1], rawJson);
		}
		...
		while(splitArr.length > 1) {
			...
			for(let i = 0; i < splitArr.length; i++) {
				...
				if(fragment.indexOf('<!--#if') === 0) {
					...
					processWrapsJson({ ret: 'content', content: fragment, path: pathStack, tmpPath: tempDirectory  }, i, splitArr);
				}
				else if(fragment.indexOf(insertPattern) === 0) {
					splitArr[i] = processInsertsJson({ ret: 'content', content: fragment, path: pathStack, tmpPath: tempDirectory })
				}
