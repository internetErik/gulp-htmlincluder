...
exports.buildWrapAST = exports.buildAST = undefined;
...
var buildAST = exports.buildAST = function buildAST(file, json) {
...
// entry point for processing files
var buildWrapAST = exports.buildWrapAST = function buildWrapAST(file, parent, json) {
...
var processNode = function processNode(node, json) {
  ...
  node.nestedNodes = contentArr[0] !== node.content ? parseNodesToAst(node, contentArr, json) : [];
...
//
var parseNodesToAst = function parseNodesToAst(parent, contentArr, json, closeTag) {
  var nodes = [];
  ...
  while (contentArr.length > 0) {
    ...
    node.children = type === 'wrap' ? parseNodesToAst(node, contentArr, json, '<!--#endwrap') : type === 'each' ? parseNodesToAst(node, contentArr, json, '<!--#endeach') : type === 'if' ? parseNodesToAst(node, contentArr, json, '<!--#endif') : [];

    // handle all nested node
    // skip if we are building up children
    if (!closeTag) processNode(node, json);
...
var resolveNode = function resolveNode(node, json) {
  ...
  if (node.nestedNodes.length > 0) {
    ...
    node.content = node.nestedNodes.map(function (c) {
      return c.content;
    }).join('');
  }
...
var nodeProcessors = {
  ...
  wrap: function wrap(node, json) {
    ...
    var wrapNode = buildWrapAST(wrapFile, node, json);
