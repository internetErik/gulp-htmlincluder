			...
			if(content.length > 1) {
				...
				content = content.map(function(fragment, ndx, arr) {
					...
					if(fragment.indexOf('<!--#jsonInsert') === 0) {
						...
						fragment = processInsertsJson({ ret: 'content', content: fragment, path: pathStack, tmpPath: tempDirectory })
					}
						...
						fragment = processWrapsJson({ ret: 'content', content: fragment, path: pathStack, tmpPath: tempDirectory  }, ndx, arr);
					}
					pathStack = file.path;
