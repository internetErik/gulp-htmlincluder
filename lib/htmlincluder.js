...
module.exports = (function htmlIncluder() {
	...
	function processInsertsJson(file) {
		...
		if(hasTagAttribute("jsonPath", content))
			jsonParentPath = getTagAttribute("jsonPath", content);


		if(hasTagAttribute("path", content)) {
	...
	function processJsonData(tag) {
		...
		jsonPath = getTagAttribute("jsonPath", tag);
		...
		while(changed) {
			...
			if(content.length > 1) {
				...
				content = content.map(function(fragment, ndx, arr) {
						...
						if(hasPath)
							tempDirectory = buildPathFromRelativePath(file.path, getTagAttribute("path", fragment));
						else if(hasAbsPath)
