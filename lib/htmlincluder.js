...
var defaultNode = (_defaultNode = {
  ...
  attributes: {} }, _defineProperty(_defaultNode, 'innerScope', {}), _defineProperty(_defaultNode, 'jsonPath', ''), _defineProperty(_defaultNode, 'json', {}), _defaultNode);

// entry point for processing files
var buildAST = exports.buildAST = function buildAST(file, json, parent, innerScope) {
...
var nodeProcessors = {
  ...
  insert: function insert(node, json) {
    ...
    var insertFile = _config.insertFiles[filename];

    // set scope for inserted file
    var newInnerScope = rawJson && jsonPath ? (console.log('1'), getDataFromJsonPath(jsonPath, rawJson)) : innerScope && jsonPath ? (console.log('2'), getDataFromJsonPath(jsonPath, innerScope)) : rawJson ? rawJson : jsonPath ? getDataFromJsonPath(jsonPath, json) : void 0;
    console.log('*** new inner scope: ', jsonPath, rawJson, innerScope);
    var insertNode = buildAST(insertFile, json, node, newInnerScope);
  ...
  wrap: function wrap(node, json) {
    ...
    var wrapNode = buildAST(wrapFile, json, node, newInnerScope);
  ...
  each: function each(node, json) {
    ...
    var values = rawJson || json;
    var jsonData = jsonPath ? getDataFromJsonPath(jsonPath, values) : void 0;
    var data = Array.isArray(values) ? values : Array.isArray(jsonData) ? getDataFromJsonPath(jsonPath, jsonData) : void 0;
    ...
    var _loop = function _loop(i) {
      ...
      var tmpChildren = node.children.map(function (c) {
        return _extends({}, c, data ? { innerScope: data[i] } : {});
      });
      // handle children content
