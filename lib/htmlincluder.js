...
function processRawJson(jsonString) {
  ...
  try {
    ...
    console.error('ERROR: Poorly formatted rawJson: ' + jsonString);
    console.error(e);
  }
    ...
    for (var i = 0; i < splitArr.length; i++) {
      ...
      if (fragment.indexOf('<!--#') === 0) {
        ...
        if (fragment.indexOf('<!--#data') === 0) {
          ...
          splitArr[i] = (0, _insert2.default)(curFile, jsonContext);
          ...
          (0, _wrap2.default)(curFile, i, splitArr, jsonContext);
        }
        ...
        if (!(0, _attributes.hasTagAttribute)('rawJson', tmp[j])) {
          if (tmp[j].indexOf('<!--#') === 0) {
            if (_typeof(jsonData[_i]) === 'object') {
              tmp[j] = [(0, _json.toSafeJsonString)(jsonData[_i]), jsonPath, file.path, tmp[j]];
